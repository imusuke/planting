...
function parsePlantDescriptionResponse(text) {
  ...
  try {
    ...
    var jsonLikeSummary = normalizePlainText(extractJsonLikeField(raw, "summary"), 4000);
    var jsonLikeBody = normalizeBodyText(extractJsonLikeField(raw, "body")).slice(0, 100000);
    if (!jsonLikeSummary) {
      jsonLikeSummary = normalizePlainText(extractJsonLikeField(text, "summary"), 4000);
    }
    if (!jsonLikeBody) {
      jsonLikeBody = normalizeBodyText(extractJsonLikeField(text, "body")).slice(0, 100000);
    }
