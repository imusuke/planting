...
const PLANT_OBSERVATION_BANNED_PATTERNS = [
  "前回の印象と比べると",
  "見どころが少し増えてきたように見えます",
  "色や形の変化が重なり",
  "季節の進み方まで伝わってくる記録",
...
const PLANT_DESCRIPTION_STYLE_BANNED_PATTERNS = [
  /〜の写真です/u,
  /ように見えます/u,
  /感じられます/u,
  /に目を向けると/u,
...
const PLANT_BODY_SECTION_SPECS = [
  {
    key: "general",
    header: "【一般的な特徴】",
    labelPattern: /^(?:【\s*(?:一般的な特徴|特徴|基本の特徴|魅力)\s*】\s*[:：]?\s*)/i,
    promptInstruction:
      "この植栽の魅力や今の見どころを、読み手にすすめるような語り口で2〜3文にまとめてください。",
    repairInstruction:
      "【一般的な特徴】として、客観的な図鑑文ではなく魅力が伝わる2〜3文に整えてください。",
  },
  {
    key: "care",
    header: "【季節ごとの手入れ】",
    labelPattern: /^(?:【\s*(?:季節ごとの手入れ|季節の手入れ|手入れ|管理のポイント)\s*】\s*[:：]?\s*)/i,
    promptInstruction:
      "春夏秋冬を意識しながら、この植栽に合う手入れのコツを実用的で読みやすい2〜3文にまとめてください。",
    repairInstruction:
      "【季節ごとの手入れ】として、春夏秋冬の流れが分かる実用的な2〜3文に整えてください。",
  },
  {
    key: "story",
    header: "【この場所での変遷】",
    labelPattern: /^(?:【\s*(?:この場所での変遷|時系列の変化|この場所でのストーリー|時系列ストーリー|変遷)\s*】\s*[:：]?\s*)/i,
    promptInstruction:
      "この場所での時系列の変化を、どこが育ち、何が見どころとして立ち上がってきたかが伝わる2〜3文で紹介してください。",
    repairInstruction:
      "【この場所での変遷】として、時系列の流れと今の見どころが自然に読める2〜3文へ整えてください。",
  },
];
...
function isValidTitle(title) {
  ...
  return (
    ...
    !/^["']/.test(value) &&
    !containsPlantDescriptionStyleNoise(value)
  );
}
...
function buildPlantBodyFromSections(sections) {
  var blocks = PLANT_BODY_SECTION_SPECS.map(function (spec) {
...
function extractPlantSectionsFromBody(body) {
  var sections = {};
  PLANT_BODY_SECTION_SPECS.forEach(function (spec) {
    sections[spec.key] = extractPlantSectionContent(body, spec);
  });
  return sections;
}

function isCaptionLikePlantObservation(text) {
  var value = collapseInlineWhitespace(text);
...
function sanitizePlantObservationText(value, options) {
  var opts = options || {};
  var text = collapseInlineWhitespace(normalizePlainText(value, 220));
  if (!text) return "";
  if (isCaptionLikePlantObservation(text)) return "";
  if (containsPlantDescriptionStyleNoise(text)) return "";
  if (!opts.allowShort && /^[^。！？]{1,18}$/.test(text)) return "";
  if (/は、丸。|から、\d{4}-\d{2}-\d{2}には/.test(text)) return "";
  if (PLANT_OBSERVATION_BANNED_PATTERNS.some(function (pattern) { return text.indexOf(pattern) !== -1; })) {
    return "";
  }
  if (
    !opts.allowShort &&
...
function containsPlantDescriptionStyleNoise(text) {
  var value = collapseInlineWhitespace(text);
  if (!value) return false;
  return PLANT_DESCRIPTION_STYLE_BANNED_PATTERNS.some(function (pattern) {
...
function firstNonEmptyObservation(record) {
  if (!record) return "";
  var note = sanitizePlantObservationText(record.note, { allowShort: true });
  if (note) return note;
  if (Array.isArray(record.photoMemos)) {
    for (var i = 0; i < record.photoMemos.length; i++) {
      var memo = sanitizePlantObservationText(record.photoMemos[i], { allowShort: false });
      if (memo) return memo;
    }
  }
  return "";
...
function buildFallbackPlantTitle(options) {
  ...
  var areaLabel = normalizePlainText(options && options.areaLabel, 120);
  var title = areaLabel ? areaLabel + "で楽しむ" + plantName : plantName + "の育ちと魅力";
  if (title.length > TITLE_MAX_CHARS) {
    title = title.slice(0, TITLE_MAX_CHARS).trim();
  }
  if (title.length < TITLE_MIN_CHARS) {
    title = plantName + "の育ちと魅力";
  }
  return collapseInlineWhitespace(title);
}
...
function buildFallbackPlantSection(spec, options) {
  ...
  if (spec.key === "general") {
    return (
      plantName +
      "の魅力は、季節が進むたびに葉姿や株のまとまりが少しずつ豊かになっていくところです。" +
      areaLabel +
      "では、" +
      (lastObservation || "葉や枝ぶりの変化") +
      "が今の見どころとして立ち上がり、この植栽を主役に眺めたくなる表情が整ってきました。" +
      "写真を追うほど、この場所で育てる面白さや、この先の変化を待ちたくなる期待まで自然にふくらみます。"
    );
  }
  if (spec.key === "care") {
    return (
      plantName +
      "は、季節ごとのリズムに合わせてひと手間かけるほど魅力が伸びやすい植栽です。" +
      "春は乾きすぎを防いで芽吹きを後押しし、夏は蒸れや傷んだ枝葉を早めに整えると、株の表情を保ちやすくなります。" +
      "秋から冬は休みに向かう枝ぶりを軽く整理し、次の季節にまた気持ちよく動き出せる土台をつくるのがおすすめです。"
    );
  }
  return (
    areaLabel +
    ...
    (firstDate || "記録の初期") +
    "ごろの" +
    (firstObservation || "まだ落ち着いた株姿") +
    "から、" +
    (lastDate || "最近") +
    "には" +
    (lastObservation || "葉や枝ぶりの広がり") +
    "へと見どころを重ねてきました。" +
    "写真を並べると、株の広がりや葉の付き方、色の出方が少しずつ積み上がり、この場所ならではの育ち方がはっきり見えてきます。" +
    plantName +
    "を一枚ずつ追うほど、次はどこが伸びるのか、どんな表情を見せるのかを楽しみにしたくなる流れです。"
  );
}
...
function buildPlantTitlePrompt(options) {
  ...
  var lines = [
    ...
    "句点で終わる説明文ではなく、続きを読みたくなる見出しとして自然な一行にしてください。",
    "客観的な分類名だけで終わらせず、この植栽の魅力や見どころがにじむ表現にしてください。",
...
function buildPlantTitleRepairPrompt(rawText) {
  return [
    ...
    "10〜48文字、一行、引用符なし、見出しとして自然で、魅力がにじむ言い回しにしてください。",
...
function normalizeGeneratedPlantBody(text) {
  ...
  paragraphs.forEach(function (paragraph) {
    ...
    for (var i = 0; i < PLANT_BODY_SECTION_SPECS.length; i++) {
      if (PLANT_BODY_SECTION_SPECS[i].labelPattern.test(paragraph)) {
        matchedSpec = PLANT_BODY_SECTION_SPECS[i];
        break;
      }
    }
  ...
  PLANT_BODY_SECTION_SPECS.forEach(function (spec, index) {
    if (assigned[spec.key]) return;
    if (!unassigned.length) return;
    if (index === PLANT_BODY_SECTION_SPECS.length - 1) {
  ...
  var sections = PLANT_BODY_SECTION_SPECS.map(function (spec) {
...
function isValidBody(body) {
  ...
  return PLANT_BODY_SECTION_SPECS.every(function (spec) {
...
function buildPlantSummaryPrompt(options) {
  var summaryPlantName = normalizePlainText(options.plantName, 120) || "この植栽";
  var summaryAreaLabel = normalizePlainText(options.areaLabel, 120) || "このエリア";
  var summaryTimelineLines = buildTimelineLines(options.timelineRecords || []);
  var summaryLines = [
    "次の活動記録をもとに、植栽ページの概要だけを書いてください。",
    "対象植栽: " + summaryPlantName,
    "エリア: " + summaryAreaLabel,
    "出力は1段落、1〜2文、45〜110文字にしてください。",
    "この植栽の今の見どころと、この場所で育てる面白さが短く伝わる概要にしてください。",
    "他人事の解説ではなく、読み手にこの植栽を紹介するつもりで主体的に書いてください。",
    "『〜の写真です』『〜ように見えます』『前回の印象と比べると』のような写真コメント風の言い回しは禁止です。",
    "引用符、見出し、JSON、箇条書き、コードブロックは禁止です。",
  ];
  if (summaryTimelineLines.length) {
    summaryLines.push("時系列メモ:");
    summaryTimelineLines.forEach(function (line) {
...
function buildPlantBodyPrompt(options) {
  var bodyPlantName = normalizePlainText(options.plantName, 120) || "この植栽";
  var bodyAreaLabel = normalizePlainText(options.areaLabel, 120) || "このエリア";
  var bodyTimelineLines = buildTimelineLines(options.timelineRecords || []);
  var bodyLines = [
    "次の活動記録をもとに、植栽ページの詳細メモだけを書いてください。",
    "対象植栽: " + bodyPlantName,
    "エリア: " + bodyAreaLabel,
    "出力は必ず次の3段落だけにしてください。段落の間は空行を1つ入れてください。",
    "1段落目は `【一般的な特徴】` で始め、この植栽の魅力や今の見どころを、こちらが勧めたくなる語り口で2〜3文にまとめてください。",
    "2段落目は `【季節ごとの手入れ】` で始め、春夏秋冬の流れに沿って、育てる人の背中を押せる実用的な手入れを2〜3文で書いてください。",
    "3段落目は `【この場所での変遷】` で始め、この場所でどう育ち、何が見どころとして立ち上がってきたかを、読み手に追ってほしくなるストーリーとして2〜3文で紹介してください。",
    "全体で420〜900文字程度にしてください。",
    "どの段落も途中で切らず、必ず句点で終えてください。",
    "『〜の写真です』『〜ように見えます』『前回の印象と比べると』のような写真コメント風の言い回しは禁止です。",
    "引用符、JSON、箇条書き、コードブロックは禁止です。",
  ];
  if (bodyTimelineLines.length) {
    bodyLines.push("時系列メモ:");
    bodyTimelineLines.forEach(function (line) {
      bodyLines.push("- " + line);
    });
  }
...
function buildPlantSummaryRepairPrompt(rawText) {
  return [
    ...
    "読み手にこの植栽を紹介するような、主体的で自然な文にしてください。",
...
function buildPlantBodyRepairPrompt(rawText) {
  return [
    ...
    "他人事の解説ではなく、読み手にこの植栽の魅力を案内する語り口にしてください。",
    "『〜ように見えます』『前回の印象と比べると』のような写真コメント風の言い回しは禁止です。",
...
function buildPlantDescriptionPrompt(options) {
  var descriptionPlantName = normalizePlainText(options.plantName, 120) || "この植栽";
  var descriptionAreaLabel = normalizePlainText(options.areaLabel, 120) || "このエリア";
  ...
  var lines = [
    ...
    "対象植栽: " + descriptionPlantName,
    "エリア: " + descriptionAreaLabel,
    "summary は1〜2文、40〜160文字程度で、この植栽の今の見どころと、この場所で育てる面白さが伝わる概要にしてください。",
    "summary は他人事の解説ではなく、読み手にこの植栽を紹介するつもりで主体的に書いてください。",
    "body は3〜6段落、350〜1100文字程度で、次の3部構成をこの順番で守ってください。",
    "第1部では、この植栽の一般的な特徴、魅力、見どころを、読み手に勧めたくなる紹介文として書いてください。",
    "第2部では、春・夏・秋・冬の季節ごとの手入れ方法を、この植栽に合う実用的な内容としてまとめてください。",
    "第3部では、この場所で育っているこの植栽が、時系列写真の中でどう変わってきたかを、追ってみたくなるストーリーとして紹介してください。",
    "『〜の写真です』『〜ように見えます』『前回の印象と比べると』のような写真コメント風の言い回しは禁止です。",
  ...
  if (firstDate || lastDate) {
    lines.push("記録期間: " + (firstDate || "不明") + " から " + (lastDate || "不明"));
  }
  if (summaryDraft) {
...
function buildPlantDescriptionRepairPrompt(rawText) {
  return [
    ...
    "全体を通して、他人事の解説ではなく、読み手にこの植栽を案内する主体的な語り口にしてください。",
    "『〜の写真です』『〜ように見えます』『前回の印象と比べると』のような写真コメント風の言い回しは禁止です。",
    "応答は次のJSONだけにしてください。",
    '{"summary":"ここに概要","body":"ここに本文"}',
