...
function buildPlantDescriptionPrompt(options) {
  ...
  var lines = [
    ...
    "body は3〜6段落、350〜1100文字程度で、次の順番を守ってください。",
    "本文の最初では、この植栽の一般的な特徴や見どころを、園芸ページの導入として自然に説明してください。",
    "その次に、春・夏・秋・冬の季節ごとの手入れの考え方を、この植栽に合う実用的な内容としてまとめてください。季節が偏る場合でも、年間を通した手入れの流れが分かるようにしてください。",
    "最後の1〜2段落では、この場所で育っているこの植栽が、時系列写真の中でどう変わってきたかをストーリーとして紹介してください。",
    "一般説明、季節ごとの手入れ、この植栽の時系列ストーリー、の順番が読んで自然に分かる構成にしてください。",
...
function buildPlantDescriptionRepairPrompt(rawText) {
  return [
    ...
    "summary は1〜2文、body は一般説明 → 季節ごとの手入れ → 時系列ストーリーの順を保った日本語の説明文にしてください。",
    "応答は次のJSONだけにしてください。",
    '{"summary":"ここに概要","body":"ここに本文"}',
