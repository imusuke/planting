...
const AREA_DESCRIPTION_STYLE_BANNED_PATTERNS = [
  /〜の写真です/u,
  /ように見えます/u,
  /感じられます/u,
  /に目を向けると/u,
...
function sanitizeAreaObservationText(value, options) {
  ...
  if (containsAreaDescriptionStyleNoise(text)) return "";
...
function containsAreaDescriptionStyleNoise(text) {
  var value = collapseInlineWhitespace(text);
  if (!value) return false;
  return AREA_DESCRIPTION_STYLE_BANNED_PATTERNS.some(function (pattern) {
...
function normalizeGeneratedAreaSummary(text, options) {
  ...
  if (containsAreaDescriptionStyleNoise(summary)) {
    summary = buildFallbackAreaSummary(options);
  }
...
function normalizeGeneratedAreaBody(text, options) {
  ...
  var blocks = splitParagraphBlocks(normalized)
    ...
    .filter(function (block) {
      return block && !isCaptionLikeParagraph(block) && !containsAreaDescriptionStyleNoise(block);
    });
  ...
  if (blocks.length < 2) {
    var sentences = splitJapaneseSentences(normalized)
      ...
      .filter(function (sentence) {
        return sentence && !isCaptionLikeParagraph(sentence) && !containsAreaDescriptionStyleNoise(sentence);
      });
...
function isValidAreaSummary(summary) {
  ...
  return (
    ...
    !/\\n/.test(value) &&
    /[。！？]$/.test(value) &&
    !containsAreaDescriptionStyleNoise(value)
  );
}
...
function isValidAreaBody(body) {
  ...
  return blocks.every(function (block) {
    ...
    return (
      ...
      !containsAreaDescriptionStyleNoise(collapsed)
    );
  });
}
...
function buildFallbackAreaBody(options) {
  ...
  var paragraph3 = ensureSentenceEnding(
    areaLabel +
      "の記録は、単発の写真として眺めるより、時間を重ねながら景色を育てていく流れで読むほど面白さが増します。" +
      "季節ごとに前へ出てくる草花や鉢の役割を追うと、この場所ならではの見せ場が少しずつはっきりしてきます。"
  );
