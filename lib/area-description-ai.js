  ...
  for (var i = list.length - 1; i >= 0; i--) {
    ...
    if (record && Array.isArray(record.photoMemos)) {
      for (var j = 0; j < record.photoMemos.length; j++) {
        var memo = normalizePlainText(record.photoMemos[j], 140);
        if (memo) return memo;
      }
    }
    var note = normalizePlainText(record && record.note, 140);
    if (note) return note;
  }
  return "";
}
...
function buildFallbackAreaSummary(options) {
  ...
  return trimToCompleteSentences(
    areaLabel + "では、" + latest + "を手がかりに、写真ごとの変化を追いやすいまとまりのある景色が育っています。",
...
function buildFallbackAreaBody(options) {
  ...
  var paragraph1 = ensureSentenceEnding(
    areaLabel +
      "は、写真を見比べることで植え込みや鉢まわりの雰囲気の移り変わりを感じやすいエリアです。" +
      "最新の記録では、" +
      lastObs +
      "が景色の中心になっていて、色のまとまりや広がり方にもこの場所らしさが表れています。"
  );
  var paragraph2 = ensureSentenceEnding(
    firstDate +
      ...
      lastObs +
      "へと景色が移ってきました。" +
      "写真ごとに葉の量や花の見え方、足元のにぎわいが少しずつ変わり、季節の進み方が読み取りやすくなっています。"
  );
  var paragraph3 = ensureSentenceEnding(
    areaLabel +
      "の記録は、単発の写真というより、時間を重ねながら景色が整っていく流れとして読むと魅力が伝わります。" +
      "前の記録と見比べることで、どこに変化が集まっているか、どの草花や鉢がその時期の主役なのかがつかみやすくなります。"
  );
...
function buildAreaBodyPrompt(options) {
  ...
  var lines = [
    ...
    "1段落目では、今の景色やこのエリアの役割を自然な紹介文として説明してください。",
    "2段落目では、写真を見比べて分かる季節の移り変わりや景色の変化を説明してください。",
    "3段落目を入れる場合は、この場所の記録を通して見えてくる流れや見どころをまとめてください。",
