...
function buildPrompt(context) {
  ...
  var lines = [
    ...
    "コメントは20文字以上100文字未満、1〜3文にしてください。",
    "1文目では、写真から見てわかる具体的な観察を書いてください。色、数、高さ、広がり、向き、ふくらみ、葉や花や実の状態などに触れてください。",
    "2文目では、見てわかる範囲で軽い考察や変化を書いてください。推測しすぎず、写真と記録文脈から読み取れる範囲だけにしてください。",
    "既存の写真メモがある場合は、その意味、観点、固有名詞、言い回しをできるだけ残し、完全に別内容へ書き換えないでください。",
    "『様子が伝わります』『記録です』『写真です』のような定型的な締め方だけで終わらせないでください。",
    "短い観察メモとして、具体的でやわらかい文にしてください。",
    "良い例: 葉が上向きにそろい、株全体に張りが出てきました。前回より緑が濃く見え、勢いが増している印象です。",
    "良い例: つぼみが数を増やし、先端のふくらみも目立ってきました。開花へ向かう動きが感じられます。",
  ];
...
function buildRepairPrompt(context, draft, reason) {
  var lines = [
    ...
    "1文目では、写真から見てわかる具体的な観察を書いてください。色、数、高さ、広がり、向き、ふくらみ、葉や花や実の状態などに触れてください。",
    "必要なら2文目で軽い考察や変化を書いてください。『様子が伝わります』『記録です』のような定型句だけで終わらせないでください。",
    "既存の写真メモがある場合は、その意味、観点、固有名詞、言い回しをできるだけ残し、完全に別内容へ書き換えないでください。",
    "下書きの問題: " + reason,
...
function polishComment(text) {
  ...
  out = out
    ...
    .replace(/成長の様子がよく伝わる写真です/g, "成長の変化が見て取れます")
    .replace(/成長の変化がよくわかります/g, "成長の変化が見て取れます")
    .replace(/での記録です/g, "を残した記録です")
    .replace(/写真です/g, "一枚です");
...
function buildFallbackSegments(context) {
  ...
  var previousPhotoMemo =
    context && context.previousPhotoMemo
      ? normalizeComment(String(context.previousPhotoMemo).slice(0, 60))
      : "";
  var plants = normalizePlantNames(context && context.plantNames);

  if (currentPhotoMemo) segments.push(currentPhotoMemo);
  if (noteText) segments.push(noteText);
  if (previousPhotoMemo) segments.push("前回との違いも追いやすい一枚です。");
  if (plants.length) segments.push(plants.join("、") + "の変化が見て取れます。");
  segments.push("葉や株の動きが見て取れます。");
  return segments;
}
...
function expandCommentToMinimum(comment, context) {
  ...
  if (out.length < COMMENT_MIN_LENGTH) {
    out = appendSentence(out, "成長の変化が見て取れます。");
  }
