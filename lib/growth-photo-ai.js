...
function buildObservationPlanPrompt(context) {
  ...
  var lines = [
    ...
    "返答は必ず次のJSONだけにしてください。",
    '{"requestedViewpoint":"見てほしい点を写真の観察に置き換えた短い説明","observations":["観察1","観察2"],"comparison":"前回比較があれば短く、なければ空文字","closingAngle":"最後に触れたい変化や読み取りを短く"}',
    ...
    "comparison は比較できる場合だけ入れてください。無理に前回比較を作らないでください。",
...
function buildCommentFromObservationPlanPrompt(context, plan) {
  ...
  var lines = [
    ...
    "comparison が空でなければ、前回比較として不自然にならない範囲で一度だけ触れてください。",
  ...
  lines.push("前回比較: " + String(plan && plan.comparison ? plan.comparison : "").slice(0, 120));
  ...
  try {
    ...
    var observations =
      parsed && Array.isArray(parsed.observations)
        ...
        : [];
    var comparison =
      parsed && typeof parsed.comparison === "string"
        ? stripSentenceTail(sanitizeMemoForAi(parsed.comparison)).slice(0, 120)
        : "";
    var closingAngle =
...
async function requestGeminiObservationPlan(opts, context) {
  var response = await requestGeminiRaw(
    opts,
    buildObservationPlanPrompt(context || {}),
    {
      temperature: 0.18,
      maxOutputTokens: 360,
    }
...
function buildCueSegments(context, seed) {
  ...
  if (noteText && noteText.length >= 8) {
    segments.push("記録に残した視点ともつながりやすく、写真の中の変化を自然に言葉へ起こしやすい場面です。");
  }
  if (currentPhotoMemo && currentPhotoMemo.length >= 8) {
    segments.push("これまで注目していた点を踏まえながら、今回は色や形の変化として見どころを組み立てやすい内容です。");
  }
  if (userInstruction && userInstruction.length >= 4) {
    if (!focusSegments.length) {
      focusSegments.push("画面の中で特に目を引く部分がはっきりしており、どこを見比べると変化が分かるかを拾いやすい一枚です。");
    }
  }
...
function buildContextLines(context) {
  ...
  if (userInstruction) {
    lines.push("ユーザーが見てほしい点（語句は写さず意図だけ拾う）: " + userInstruction.slice(0, 800));
    var focusSummary = summarizeUserInstructionFocus(context);
    if (focusSummary) {
...
function summarizeUserInstructionFocus(context) {
  var terms = extractUserInstructionFocusTerms(context && context.userInstruction);
  return terms.length ? terms.slice(0, 3).join("、") : "";
}

function hasComparisonFocusTerm(terms) {
  for (var i = 0; i < terms.length; i++) {
    if (USER_INSTRUCTION_COMPARISON_TERMS[terms[i]]) return true;
  }
  return false;
}
...
function pickPrimaryUserInstructionFocus(terms) {
  ...
  for (var i = 0; i < list.length; i++) {
    if (!USER_INSTRUCTION_COMPARISON_TERMS[list[i]]) return list[i];
  }
...
function buildUserInstructionCueSegments(context) {
  ...
  var secondary = "";
  for (var i = 0; i < terms.length; i++) {
    if (terms[i] !== primary && !USER_INSTRUCTION_COMPARISON_TERMS[terms[i]]) {
      secondary = terms[i];
      break;
    }
  }
  ...
  if (primary && secondary) {
    segments.push(primary + "や" + secondary + "に目を向けると、今回の写真で拾うべき見どころが自然に浮かび上がってきます。");
    segments.push(primary + "だけでなく" + secondary + "まで追うと、画面の中で変化が集まっている場所をつかみやすくなります。");
  } else if (primary) {
    segments.push(primary + "に目を向けると、今回の写真で押さえたい見どころが自然に浮かび上がってきます。");
    segments.push(primary + "の見え方が画面の中で効いており、ほかの部分との違いまで拾いやすい一枚です。");
  }

  if (hasComparisonFocusTerm(terms)) {
    segments.push("前回との違いまで意識して見ると、今回どこに変化が集まっているかを整理しやすい一枚です。");
  }
...
function getValidationError(comment, context) {
  ...
  if (/(補足メモ|ユーザー|見てほしい点|入力メモ|写真メモ|記録メモ|指示|要望|気になっている見方)/.test(text)) {
