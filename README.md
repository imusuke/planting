...
5. 写真コメントや AI 下書きを使うなら、環境変数 **`GEMINI_API_KEY`** に Google AI Studio / Gemini API のキーを設定する。モデルは参考実装 `agentic vision` に合わせて **`GEMINI_MODEL`** を優先し、既定値は **`gemini-3-flash-preview`** です。互換用に **`GEMINI_PHOTO_COMMENT_MODEL`** も読めます。植栽説明だけ別モデルにしたい場合は **`GEMINI_PLANT_DESCRIPTION_MODEL`** を設定すると、`plant-edit.html` の `AIで植栽説明を作成` がそのモデルを優先して使います。植栽説明は `タイトル / 一般的な特徴 / 季節ごとの手入れ / この場所での変遷` を別々に生成してから組み立てます。編集ページでは、写真追加やメモ更新の内容を保存すると、その保存済みデータに対して Gemini がサーバー側でコメント案をバックグラウンド更新します。必要なら `保存後にバックグラウンドで再生成` ボタンで全写真の再生成も予約できます。API は `GROWTH_UPLOAD_TOKEN` と同じトークンで保護されます。
